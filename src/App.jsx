...
function App() {
  ...
  return (
    ...
    {getuser ? (
        <div>
          <h1>Hello, {getuser.user.email}!</h1>
          <SignOut />
         ...
         <li key={todo.id} className={`list-item ${todo.isCompleted ? 'line-through' : ''} bg-white flex items-center shadow-lg mb-2 p-4 rounded decoration-red-900`}>
           {isEditing === todo.data.completed ? (
             ...
             <div className="todo-container flex justify-between items-center">
              ...
              <div>
              <button className="btn-alert bg-amber-500 hover:bg-yellow-700 text-white py-2 px-4 rounded-l" onClick={() => openEditModal(todo)}>Edit</button>
